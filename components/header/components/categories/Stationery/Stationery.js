...
class Stationery extends HTMLElement {
  constructor() {
    super();
  ...
  async connectedCallback() {
    const [baseStyles, customStyles] = await Promise.all([
      ...
      fetch(new URL("./Stationery.desktop.css", import.meta.url)).then((res) =>
        res.text()
      )
    ]);

    const category = categoryNames.STATIONERY;
    const subcategory = subcategoryNames.STATIONERY;
...
customElements.define("stationery-category", Stationery);
