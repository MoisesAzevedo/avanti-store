...
class Beauty extends HTMLElement {
  constructor() {
    super();
  ...
  async connectedCallback() {
    const [baseStyles, customStyles] = await Promise.all([
      ...
      fetch(new URL("./Beauty.desktop.css", import.meta.url)).then((res) =>
        res.text()
      )
    ]);

    const category = categoryNames.BEAUTY;
    const subcategory = subcategoryNames.BEAUTY;
...
customElements.define("beauty-category", Beauty);
