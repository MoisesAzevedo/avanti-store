...
class Clothes extends HTMLElement {
  ...
  async connectedCallback() {
    const [baseStyles, customStyles] = await Promise.all([
      ...
      fetch(new URL("./Clothes.desktop.css", import.meta.url)).then((res) =>
        res.text()
      )
    ]);

    const category = categoryNames.CLOTHES;
    const subcategory = subcategoryNames.CLOTHES;
...
customElements.define("clothes-category", Clothes);
