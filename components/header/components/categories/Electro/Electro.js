...
class Electro extends HTMLElement {
  ...
  async connectedCallback() {
    const [baseStyles, customStyles] = await Promise.all([
      ...
      fetch(new URL("./Electro.desktop.css", import.meta.url)).then((res) =>
        res.text()
      )
    ]);

    const category = categoryNames.ELECTRO;
    const subcategory = subcategoryNames.ELECTRO;
...
customElements.define("electro-category", Electro);
