...
class Market extends HTMLElement {
  constructor() {
    super();
  ...
  async connectedCallback() {
    const [baseStyles, customStyles] = await Promise.all([
      ...
      fetch(new URL("./Market.desktop.css", import.meta.url)).then((res) =>
        res.text()
      )
    ]);

    const category = categoryNames.MARKET;
    const subcategory = subcategoryNames.MARKET;
...
customElements.define("market-category", Market);
