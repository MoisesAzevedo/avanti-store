...
class Toys extends HTMLElement {
  ...
  async connectedCallback() {
    const [baseStyles, customStyles] = await Promise.all([
      ...
      fetch(new URL("./Toys.desktop.css", import.meta.url)).then((res) =>
        res.text()
      )
    ]);

    const category = categoryNames.TOYS;
    const subcategory = subcategoryNames.TOYS;
...
customElements.define("toys-category", Toys);
