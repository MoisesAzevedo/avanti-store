...
class Sale extends HTMLElement {
  ...
  async connectedCallback() {
    const [baseStyles, customStyles] = await Promise.all([
      ...
      fetch(new URL("./Sale.desktop.css", import.meta.url)).then((res) =>
        res.text()
      )
    ]);

    const category = categoryNames.SALE;
    const subcategory = subcategoryNames.SALE;
...
customElements.define("sale-category", Sale);
