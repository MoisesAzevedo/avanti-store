...
class Pets extends HTMLElement {
  ...
  async connectedCallback() {
    const [baseStyles, customStyles] = await Promise.all([
      ...
      fetch(new URL("./Pets.desktop.css", import.meta.url)).then((res) =>
        res.text()
      )
    ]);

    const category = categoryNames.PETS;
    const subcategory = subcategoryNames.PETS;
...
customElements.define("pets-category", Pets);
